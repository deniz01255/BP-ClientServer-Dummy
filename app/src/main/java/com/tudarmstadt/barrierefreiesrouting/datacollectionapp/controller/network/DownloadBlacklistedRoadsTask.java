 ...
 */
public class DownloadBlacklistedRoadsTask {

    public static void downloadBlacklistedWays() {
        OkHttpClient client = new OkHttpClient();

        Request request = new Request.Builder()
                .url(RoutingServerAPI.baseURL + RoutingServerAPI.blacklistedRoadResource)
                .build();
        ...
        client.newCall(request)
                .enqueue(new Callback() {
                    ...
                    public void onResponse(Call call, final Response response) throws IOException {
                        EventBus.getDefault().post(new BlacklistedRoadsTaskDownloadedEvent(response));
                    }
                });
    }
